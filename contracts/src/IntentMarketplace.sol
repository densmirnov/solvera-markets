...
contract IntentMarketplace {
    ...
    event Accepted(
        bytes32 indexed id, address indexed solver, uint256 rewardAmount, uint256 feeAmount, uint256 bondAmount
    );

    event Expired(bytes32 indexed id, State indexed fromState, uint256 feeAmount, uint256 refundAmount);

    event ReputationUpdated(address indexed solver, int256 newValue, ReputationReason reason);
    ...
    function createIntent(
        ...
        if (
            payer == address(0) || initiator == address(0) || verifier == address(0) || tokenOut == address(0)
        ...
        emit IntentCreated(
            id, payer, initiator, verifier, tokenOut, minAmountOut, rewardToken, rewardAmount, ttlSubmit, ttlAccept
        );
    }
    ...
    function expire(bytes32 id) external {
        ...
        if (intent.state == State.OPEN) {
            ...
            if (refundAmountOpen > 0) {
                _safeTransfer(IERC20(intent.rewardToken), intent.payer, refundAmountOpen);
            }
        ...
        if (feeAmountSelected > 0) {
            _safeTransfer(IERC20(intent.rewardToken), feeRecipient, feeAmountSelected);
        }
        if (refundAmountSelected > 0) {
            _safeTransfer(IERC20(intent.rewardToken), intent.payer, refundAmountSelected);
        }

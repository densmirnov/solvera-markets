                    ...
                    <table className="w-full caption-bottom text-sm">
                        <thead className="[&_tr]:border-b">
                            <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                ...
                                <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Min Output</th>
                                <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Initiator</th>
                        ...
                        <tbody className="[&_tr:last-child]:border-0">
                            {loading ? (
                                ...
                                data.map((intent) => {
                                    const reward = formatTokenAmount(intent.rewardAmount, intent.rewardToken);
                                    const minOut = formatTokenAmount(intent.minAmountOut, intent.tokenOut);
                                    return (
                                        <tr
                                            key={intent.id}
                                        ...
                                        >
                                            ...
                                            <td className="p-4 align-middle">
                                                <span className={cn(
                                                    ...
                                                    intent.state === "OPEN" ? "border-transparent bg-primary text-primary-foreground" : "text-foreground"
                                                )}>
                                                    {intent.state}
                                                </span>
                                            </td>
                                            <td className="p-4 align-middle">
                                                ...
                                                <div className="text-xs text-muted-foreground">{reward.secondary}</div>
                                            </td>
                                            <td className="p-4 align-middle">
                                                <div className="font-medium">{minOut.primary}</div>
                                                <div className="text-xs text-muted-foreground">{minOut.secondary}</div>
                                            </td>
                                            ...
                                            <td className="p-4 align-middle">
                                                {/* Link to detail view if we implement it, or just copy ID */}
